function calcular() {
  ...
  if (isNaN(n1) || isNaN(n2) || isNaN(n3)) {
    ...
    if (n3 == 0) {
      s = (n1 + n2) * 0.25;
      f = (6 - s) / 0.5;
      notafinal = (f - 1).toFixed(2);
      res.innerHTML = `<p>A sua nota atualmente é: <strong>${s}</strong>.<p>`;
      res.innerHTML += `<p>Para chegar a média de 6.0.</br> Ainda é preciso tirar <strong>${notafinal}</strong> na prova final. (Descosiderando a nota do BONUS)</p>`;
    } else {
